...
public class SQLiteToExcel {
    ...
    private int idform; // esto se agrego a la libreria para poder usarla a mi modo xjxjxj
    ...
    public SQLiteToExcel(Context context, String dbName, String exportPath,int idform) {
        this.idform=idform;
        mExportPath = exportPath;
        try {
            database = SQLiteDatabase.openOrCreateDatabase(context.getDatabasePath(dbName).getAbsolutePath(), null);
    ...
    private void insertItemToSheet(String table, HSSFSheet sheet, ArrayList<String> columns) {
        ...
        Cursor cursor = database.rawQuery("select * from " + table+" where idForm="+idform, null);
        cursor.moveToFirst();
        int n = 1;
